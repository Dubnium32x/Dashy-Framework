...
function SoundManager:preloadMusic()
    ...
    for key, path in pairs(musicFiles) do
        ...
        local mp3Path = path .. ".mp3"
        local wavPath = path .. ".wav"
        
        -- Try MP3 first
        print("Attempting to load MP3:", mp3Path)
        filePlayer = snd.fileplayer.new(mp3Path)
        
        -- If MP3 failed, try WAV
        if not filePlayer then
            print("MP3 load failed, trying WAV:", wavPath)
            filePlayer = snd.fileplayer.new(wavPath)
        end
        
        -- Set up the player if we successfully loaded a file
        if filePlayer then
            ...
            print("Successfully loaded music track:", key)
        else
            print("ERROR: Failed to load music track:", key)
        end
    end
