...
function InitScreen:init()
    ...
    self.t = 0
    self.logoX = 200 -- Centered horizontally
    self.logoY = 300 -- Start off-screen bottom
    self.logoScale = 0.2 -- Start small
    self.logoAngle = 0   -- Start angle for spinning
...
function InitScreen:update()
    if not self.animationDone then
        self.t = self.t + 1/45
        -- Raise up and scale up over 1 second
        local progress = math.min(self.t, 1)
        self.logoY = 300 - (300 - 135) * progress -- from 300 to 135
        self.logoScale = 0.2 + 0.8 * progress     -- from 0.2 to 1.0
        self.logoAngle = 720 * (1 - (1 - progress)^2) -- 2 full spins, ease out
        if self.t >= 1 then
