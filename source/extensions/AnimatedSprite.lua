...
---@param stateName string|nil Optional animation state name to play
---@return boolean success True if animation was successfully started
function AnimatedSprite:playAnimation(stateName)
    if stateName then
        -- Check if requested state exists
        if not self.states[stateName] then
            print("[AnimatedSprite] Warning: Animation state '" .. stateName .. "' doesn't exist.")
            return false
        end
    ...
    if (type(self.currentState) == 'nil') then
        ...
        if not state then
            print("[AnimatedSprite] Error: Default state '" .. self.defaultState .. "' doesn't exist.")
            return false
        end
...
local function addState(self, params)
	if not params or not params.name then
		print("[AnimatedSprite] Warning: Skipping unnamed animation state.")
		return false
	end
...
function AnimatedSprite:setStates(states, animate, defaultState)
	...
	if type(states) == "table" then
		...
		print("[AnimatedSprite] Warning: 'states' passed to setStates is not a table. Got type:", type(states))
	end
